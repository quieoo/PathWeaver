...
class KblamLlamaAttention(nn.Module):
    ...
    #         # # 修改
    #         # # 扩展索引用于 gather keys/values
    #         # key_value_idx = top_idx.view(batch_size, 1, topk_size, 1).expand(
    #         #     batch_size, num_heads, topk_size, head_dim
    #         # )
    #         # kb_keys = kb_keys.gather(-2, key_value_idx)
    #         # kb_values = kb_values.gather(-2, key_value_idx)
    #         # # 扩展索引用于 gather attn_weights
    #         # attn_idx = top_idx.view(batch_size, 1, 1, topk_size).expand(
    #         #     batch_size, num_heads, attn_weights.size(2), topk_size
    #         # )
    #         # attn_weights = attn_weights.gather(-1, attn_idx)
    #     return kb_keys, kb_values, attn_weights[..., :topk_size]
    
    def prune_key_value(self, query, kb_keys, kb_values, topk_size=20):
        """
        根据 query 与 kb_keys 的注意力分布，筛选 topk 的 kb tokens
        - prefill 阶段: query_len > 1
        - decode 阶段: query_len = 1
        """
        assert query.requires_grad is False, "This function should only be used at test time"
        batch_size, num_heads, kb_len, head_dim = kb_keys.shape
        _, _, q_len, _ = query.shape

        # [batch, heads, q_len, kb_len]
        ...
        with torch.autograd.no_grad():
            if q_len > 1:
                # -------- Prefill 阶段 (多 token) --------
                # 跨 heads 和 query_len 聚合，找全局 top-k kb tokens
                mean_attn = attn_weights.mean(dim=(1, 2))  # [batch, kb_len]
                top_idx = mean_attn.topk(topk_size, dim=-1)[1]  # [batch, topk]
            else:
                # -------- Decode 阶段 (单 token) --------
                # 直接对最后一个 query 的注意力做平均
                last_attn = attn_weights.squeeze(2)        # [batch, heads, kb_len]
                mean_attn = last_attn.mean(dim=1)          # [batch, kb_len]
                top_idx = mean_attn.topk(topk_size, dim=-1)[1]  # [batch, topk]
            ...
            # gather attn_weights
            attn_idx = top_idx.view(batch_size, 1, 1, topk_size).expand(
                batch_size, num_heads, q_len, topk_size
            )
            attn_weights = attn_weights.gather(-1, attn_idx)
    ...
    def forward(
        ...
        **kwargs,
        ...
        if kb_kvs is not None:
            if self.layer_idx % kb_layer_frequency == 0:
                ...
                if len(kb_keys.shape) == 2:  # Not batch dim
                    ...
                    if dynamic_sparsify:
                        # print("Dynamic sparsification of KB keys/values\n")
            ...
            if kb_kvs is not None:
                if self.layer_idx % kb_layer_frequency == 0:
                    ...
                    if attn_weights_2 is None:
                        # print(f"[Layer {self.layer_idx}] query_states_2.sum(): {query_states_2.sum().item()}")
                        # print(f"[Layer {self.layer_idx}] kb_keys.sum(): {kb_keys.sum().item()}")
                        attn_weights_2 = torch.matmul(
                            query_states_2, kb_keys.transpose(2, 3)
                        ) / math.sqrt(self.head_dim)
                        # ### 增加：单独保存KB部分的注意力得分 ###
                        # # print(f"\nattn_weights_2.shape = {attn_weights_2.shape}\n")
                        # # 对prefill阶段保存注意力得分
                        # if(query_states_2.shape[2]>1):
                        #     kb_attention_save_loc = "/home/sdu/KBLaM/KBLaM/experiments/preliminary_experiments/select-kbsize100"
                        #     os.makedirs(kb_attention_save_loc, exist_ok=True)
                        #     kb_attention_file_base_name = "attn_softmax"
                        #     save_path = os.path.join(kb_attention_save_loc, f"{kb_attention_file_base_name}_{self.layer_idx}.csv")
                        #     # scale操作
                        #     # if kb_scale_factor is not None:
                        #     #     attn_weights_2_scale = (
                        #     #         attn_weights_2 - np.log(kb_len) + np.log(kb_scale_factor)
                        #     #     )
                        #     # softmax操作
                        #     attn_weights_2_softmax = nn.functional.softmax(attn_weights_2, dim=-1, dtype=torch.float32)
                        #     # 将 attn_weights_2_softmax 转成 numpy 格式并保存成 CSV
                        #     attn_weights_2_np = attn_weights_2_softmax.to(torch.float32).cpu().detach().numpy()
                        #     # 检查维度，若为 3D（如 batch, head, kv_len），reshape 成 2D 后保存
                        #     if attn_weights_2_np.ndim > 2:
                        #     # 例如: [1, 8, 100] -> [8, 100]
                        #         # attn_weights_2_np = attn_weights_2_np.reshape(-1, attn_weights_2_np.shape[-1])
                        #         # 移除 batch 维（第 0 维），得到 [num_heads, seq_len, kb_len]
                        #         attn_weights_2_np = attn_weights_2_np[0]
                        #         # 对 head 维（第 0 维）求平均，得到 [seq_len, kb_len]
                        #         attn_weights_2_np = attn_weights_2_np.mean(axis=0)
                        #         # 取单个head
                        #         ###------###
                        #         # num_heads = attn_weights_2_np.shape[0]
                        #         # for head_id in range(num_heads):
                        #         #     per_head_attn = attn_weights_2_np[head_id]  # shape: [seq_len, kb_len]
                        #         #     save_path = os.path.join(
                        #         #                 kb_attention_save_loc, f"{kb_attention_file_base_name}_layer{self.layer_idx}_head{head_id}.csv"
                        #         #                 )
                        #         #     with open(save_path, 'a') as f:
                        #         #         f.write("###--------------###\n")
                        #         #         np.savetxt(f, per_head_attn, delimiter=",", fmt="%.4f")
                        #         ###------###
                        #     # print(f"\nattn_weights_2_np.shape = {attn_weights_2_np.shape}\n")
                        #     with open(save_path, 'a') as f:
                        #         f.write("###--------------###\n")
                        #         np.savetxt(f, attn_weights_2_np, delimiter=",", fmt="%.4f")
                        #     # np.savetxt(save_path, attn_weights_2_np, delimiter=",", fmt="%.4f")
                        #     # print(f"\n-----第{self.layer_idx}层的KB部分注意力计算完成并保存, KB長度為{kb_len}-----\n")
                        # ### 增加：单独保存KB部分的注意力得分 ###
        ...
        # ### 增加：保存KB部分和prompt部分的注意力得分 ###
        # print(f"\nattn_weights.shape = {attn_weights.shape}\n")
        # # 对prefill阶段保存注意力得分
        # if q_len > 1:
        #     kb_attention_save_loc = "/home/sdu/KBLaM/KBLaM/experiments/preliminary_experiments/kb_prompt-kbsize10"
        #     os.makedirs(kb_attention_save_loc, exist_ok=True)
        #     kb_attention_file_base_name = "attn_softmax_all"
        #     save_path = os.path.join(kb_attention_save_loc, f"{kb_attention_file_base_name}_{self.layer_idx}.csv")
        #     # 将 attn_weights 转成 numpy 格式并保存成 CSV
        #     attn_weights_np = attn_weights.to(torch.float32).cpu().detach().numpy()
        #     # 检查维度，若为 3D（如 batch, head, kv_len），reshape 成 2D 后保存
        #     if attn_weights_np.ndim > 2:
        #         # 移除 batch 维（第 0 维），得到 [num_heads, seq_len, kv_len]
        #         attn_weights_np = attn_weights_np[0]
        #         # 对 head 维（第 0 维）求平均，得到 [seq_len, kv_len]
        #         attn_weights_np = attn_weights_np.mean(axis=0)
        #         print(f"\nattn_weights_np.shape = {attn_weights_np.shape}\n")
        #         with open(save_path, 'a') as f:
        #             f.write("###--------------###\n")
        #             np.savetxt(f, attn_weights_np, delimiter=",", fmt="%.4f")
        #         # np.savetxt(save_path, attn_weights_2_np, delimiter=",", fmt="%.4f")
        #         # print(f"\n-----第{self.layer_idx}层的注意力计算完成并保存-----\n")
        # ### 增加：保存KB部分和prompt部分的注意力得分 ###
