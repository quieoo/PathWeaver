...
SCRIPT="DAG_KV_SubgraphRAG_trainable.py"
PYTHON_BIN="python"
ST_MODEL="/home/sdu/zhu/models/bge-en-v1.5/"

OUTDIR="/mnt/n0/KBLAM/KBLaM/experiments/subgraph_mlp/sweep_train_infer"
CACHE_PATH="/mnt/n0/KBLAM/KBLaM/experiments/subgraph_mlp/training_data_cache.pkl"
...
# =========================================
HIDDEN_DIM_LIST=(512 768)
LR_LIST=(1e-3 5e-4)
NEG_POS_RATIO_LIST=(4 6)
...
# Summary header
# =========================================
echo -e "train_id\tinfer_id\thidden_dim\tlr\tneg_pos_ratio\tseed_edge_topk\tper_src_cap\ttopic_top_k\tbest_dev_f1\tanswer_recall\tgraph_recall\tnone_sink_recall\tckpt\toutput_jsonl\ttrain_log\tinfer_log" > "${SUMMARY_TSV}"
...
for HIDDEN_DIM in "${HIDDEN_DIM_LIST[@]}"; do
  for LR in "${LR_LIST[@]}"; do
    for NEG_POS_RATIO in "${NEG_POS_RATIO_LIST[@]}"; do
      ...
      TRAIN_TAG=$(printf "train_%02d_hd%s_lr%s_neg%s" \
        "${train_run_id}" "${HIDDEN_DIM}" "${LR}" "${NEG_POS_RATIO}")
