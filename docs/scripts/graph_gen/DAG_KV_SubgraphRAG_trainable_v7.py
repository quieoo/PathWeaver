...
def generate_programmatic_bidirectional_kvs(
    s: str,
    o: str,
    rel: str,
    ttype: str,
    ...
    """
    Programmatically generate exactly two KV pairs for each triple.

    mode:
      - 'aggressive': more structural / schema-like
      - 'templated' : more natural QA-friendly templates
    """
    s = norm_text(s)
    o = norm_text(o)
    rel = norm_text(rel)
    ttype_norm = norm_text(ttype).upper()

    if not s or not o:
        return []

    if not rel:
        rel = 'related_to' if ttype_norm == 'RELATION' else 'attribute'

    # ---------------------------------------------------------
    # Mode 1: aggressive (your previous v7-style deterministic KV)
    # ---------------------------------------------------------
    if mode == 'aggressive':
        type_prefix = 'relation' if ttype_norm == 'RELATION' else 'attribute'
        forward_key = norm_text(f'{type_prefix} | {s} | {rel}')
        backward_key = norm_text(f'{type_prefix} | {o} | reverse_{rel}')
        return [
            {'key_string': forward_key, 'value_string': o},
            {'key_string': backward_key, 'value_string': s},
        ]

    # ---------------------------------------------------------
    # Mode 2: templated (less aggressive, more natural)
    # ---------------------------------------------------------
    if ttype_norm == 'ATTRIBUTE':
        # 属性三元组:
        #   正向: "the {relation} of {subject} is" -> value = object
        #   反向: "the entity whose {relation} is {object} is" -> value = subject
        forward_key = norm_text(f'the {rel} of {s} is')
        backward_key = norm_text(f'the entity whose {rel} is {o} is')
    else:
        # 关系三元组:
        #   正向: "{subject} {relation}" -> value = object
        #   反向: "the entity that {relation} {object} is" -> value = subject
        #
        # 这样比 reverse_{rel} 更自然，也比简单 "what {rel} {o}" 更稳定，
        # 不强依赖 relation 本身必须是严格动词短语。
        forward_key = norm_text(f'{s} {rel}')
        backward_key = norm_text(f'the entity that {rel} {o} is')
...
def build_kvedge_graph(
    ...
    program_kv_mode: str = 'templated',
) -> Tuple[List[str], Dict[int, KVEdge], Dict[int, List[int]], Dict[int, List[int]]]:
    ...
    for title, tri in iter_triples(sample, supporting_only=supporting_only):
        ...
        # Ignore original tri['kv_lists']; only use triple fields.
        kvs = generate_programmatic_bidirectional_kvs(
            s=s,
            o=o,
            rel=rel,
            ttype=ttype,
            mode=program_kv_mode,
        )
        ...
        for kv_idx, kv in enumerate(kvs):
            ...
            # First KV = forward, second KV = backward
            if kv_idx == 0:
...
def collect_training_examples(
    ...
    for sample in tqdm(used_samples, desc='Pass1: collect texts / graphs'):
        ...
        node_names, kvedges, out_adj, in_adj = build_kvedge_graph(sample, supporting_only=False, program_kv_mode=args.program_kv_mode)
        if not kvedges:
            continue
...
def create_dag_with_model(
    ...
    for batch_start in tqdm(
        ...
        for sample in sample_batch:
            ...
            node_names, kvedges, out_adj, in_adj = build_kvedge_graph(sample, supporting_only=args.supporting_only, program_kv_mode=args.program_kv_mode)
            if not kvedges:
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        default='templated',
        choices=['aggressive', 'templated'],
        help='How to programmatically generate bidirectional KV pairs from triples.',
    )

    # v7: this file always ignores original kv_lists and uses programmatic bidirectional KV generation
