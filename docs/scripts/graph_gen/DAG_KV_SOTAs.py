...
def _empty_dag_sample(sample: Dict[str, Any], reason: str) -> Dict[str, Any]:
    sample = dict(sample)
    sample['dag'] = {'kv_nodes': [], 'adj': [], 'meta': {'reason': reason}}
    return sample
...
class InferStats:
    ...
    answer_recall: int = 0
    graph_recall: int = 0
    none_sink_recall: int = 0
    miss_sink_but_in_graph: int = 0
    miss_due_to_answer_node_has_outgoing: int = 0
    miss_due_to_other_reason: int = 0
    
    # outgoing-edge type distribution on answer nodes of failed samples
    fail_answer_out_attr = 0
    fail_answer_out_rel = 0
    fail_answer_out_other = 0


def analyze_answer_placement(
    kept_nodes: Set[int],
    kept_edges: Set[int],
    kvedges: Dict[int, KVEdge],
    answer: str,
    stats: Optional[InferStats] = None,
    answer_in_full_graph: Optional[bool] = None,
) -> Dict[str, Any]:
    outdeg = defaultdict(int)
    for eid in kept_edges:
        e = kvedges[eid]
    ...
    ansN = norm_match(answer)
    answer_nodes_in_graph: Set[int] = set()
    if ansN:
        for eid in kept_edges:
            e = kvedges[eid]
            vN = norm_match(e.value)
    ...
    answer_matched = any(s in answer_nodes_in_graph for s in sinks)
    answer_in_graph = len(answer_nodes_in_graph) > 0
    has_outgoing_answer_node = any(outdeg.get(n, 0) > 0 for n in answer_nodes_in_graph)
    ...
    if stats is not None:
        ...
        if answer_in_full_graph:
            stats.graph_recall += 1
        if answer_matched:
            stats.answer_recall += 1
        if answer_in_graph:
            stats.none_sink_recall += 1
        if answer_in_graph and not answer_matched:
            stats.miss_sink_but_in_graph += 1
            if has_outgoing_answer_node:
                stats.miss_due_to_answer_node_has_outgoing += 1
                ...
                for eid in kept_edges:
                    ...
                    if e.src in answer_nodes_in_graph:
                        ...
                        if t == "ATTRIBUTE":
                            stats.fail_answer_out_attr += 1
                        elif t == "RELATION":
                            stats.fail_answer_out_rel += 1
                        else:
                            stats.fail_answer_out_other += 1
            else:
                stats.miss_due_to_other_reason += 1



def _print_infer_stats(stats: InferStats) -> None:
    if stats.total <= 0:
        return
    print(f"Answer recall: {stats.answer_recall / stats.total:.4f}")
    print(f"Graph  recall: {stats.graph_recall / stats.total:.4f}")
    print(f"None-sink recall: {stats.none_sink_recall / stats.total:.4f}")
    print(f"Miss sink but in graph: {stats.miss_sink_but_in_graph}")
    if stats.miss_sink_but_in_graph > 0:
        print(
            "Among sink-miss samples, due to answer node having outgoing edges: "
            f"{stats.miss_due_to_answer_node_has_outgoing} / {stats.miss_sink_but_in_graph} "
            f"= {stats.miss_due_to_answer_node_has_outgoing / stats.miss_sink_but_in_graph:.4f}"
        )
        print(
            "Among sink-miss samples, other reasons: "
            f"{stats.miss_due_to_other_reason} / {stats.miss_sink_but_in_graph} "
            f"= {stats.miss_due_to_other_reason / stats.miss_sink_but_in_graph:.4f}"
        )
        print(
            "Among sink-miss samples, failed answer nodes have outgoing edges: "
            f"{stats.fail_answer_out_attr + stats.fail_answer_out_rel + stats.fail_answer_out_other} "
            f"= {stats.fail_answer_out_attr / stats.miss_sink_but_in_graph:.4f} "
            f"{stats.fail_answer_out_rel / stats.miss_sink_but_in_graph:.4f} "
            f"{stats.fail_answer_out_other / stats.miss_sink_but_in_graph:.4f}"
        )


def _build_goal_ids(kv_nodes: List[Dict[str, Any]], answer: str) -> List[int]:
    goal_ids: List[int] = []
    if not answer:
        return goal_ids
    for i, kv in enumerate(kv_nodes):
...
def _run_subgraphrag_variant(
    ...
    return {
        ...
        'seed_node_ids': [int(x) for x in topic_nodes],
        'seed_entities': [node_names[i] for i in topic_nodes if 0 <= i < len(node_names)],
        'meta_extra': {'scorer': 'trainable_subgraphrag_mlp'},
    }


def _run_hipporag_variant(
    args,
    sample: Dict[str, Any],
    embedder: SentenceTransformer,
) -> Dict[str, Any]:
    question = norm_text(sample.get('question', ''))
    node_names, out_adj, kvedges = build_kvedge_graph_hippo(
        sample=sample,
        embedder=embedder,
        batch_size=args.batch_size,
    ...
    seed_ids, seed_scores, _ = get_seed_nodes_hippo(
        question=question,
        node_names=node_names,
        embedder=embedder,
        batch_size=args.batch_size,
    ...
    kept_nodes, kept_edges = select_top_edges_with_connectivity_hippo(
    ...
    if args.max_sinks is not None and args.max_sinks > 0:
        kept_nodes, kept_edges = enforce_max_sinks_entity_hippo(
            seeds=seed_nodes,
            max_sinks=args.max_sinks,
            kept_nodes=kept_nodes,
            kept_kvedges=kept_edges,
            kvedges=kvedges,
        )
    if not kept_edges:
        return {
            'reason': 'empty_after_sink_prune',
            'node_names': node_names,
            'kvedges': kvedges,
        }
    ...
    return {
        ...
        'meta_extra': {'variant': args.variant},
    }
...
def create_dag(
    ...
    samples: List[Dict[str, Any]],
    embedder: SentenceTransformer,
    model: Optional[MLPScorer] = None,
    device: Optional[torch.device] = None,
    verbose: bool = False,
) -> List[Dict[str, Any]]:
    if args.limit is not None and args.limit > 0:
        samples = samples[:args.limit]
    if verbose and len(samples) >= 3:
        samples = [samples[2]]

    out_samples: List[Dict[str, Any]] = []
    stats = InferStats()
    desc = f'Create DAG ({args.variant})'
    ...
    for sample in tqdm(samples, desc=desc):
        ...
        if args.variant == 'subgraphrag':
            if model is None or device is None:
                raise ValueError('subgraphrag inference requires a loaded model and device.')
            result = _run_subgraphrag_variant(args, sample, embedder, model, device)
        else:
            result = _run_hipporag_variant(args, sample, embedder)

        if result.get('reason') == 'no_kv_edges':
            out_samples.append(_empty_dag_sample(sample, 'no_kv_edges'))
            continue

        node_names = result['node_names']
        kvedges = result['kvedges']
        answer_in_full_graph = any(value_matches_answer(e.value, answer) for e in kvedges.values())

        if result.get('reason') is not None:
            stats.total += 1
            if answer_in_full_graph:
                stats.graph_recall += 1
            out_samples.append(_empty_dag_sample(sample, result['reason']))
            continue

        kept_nodes = result['kept_nodes']
        kept_edges = result['kept_edges']
        analyze_answer_placement(
            kept_nodes=kept_nodes,
            kept_edges=kept_edges,
            kvedges=kvedges,
            answer=answer,
            stats=stats,
            answer_in_full_graph=answer_in_full_graph,
        )
        ...
        meta = {
            ...
            'num_kv_edges': int(len(kept_edges)),
            'num_kv_nodes': int(len(kv_nodes)),
            'goal_ids': _build_goal_ids(kv_nodes, answer),
            'seed_node_ids': list(result.get('seed_node_ids', [])),
            'seed_entities': list(result.get('seed_entities', [])),
            **result.get('meta_extra', {}),
        }

        sample_out = dict(sample)
        sample_out['dag'] = {'kv_nodes': kv_nodes, 'adj': adj, 'meta': meta}
        out_samples.append(sample_out)

    _print_infer_stats(stats)
    return out_samples
...
def main():
    ...
    if args.mode == 'train' and args.variant != 'subgraphrag':
        raise ValueError('--mode train is only supported for --variant subgraphrag.')
    ...
    if args.variant == 'subgraphrag':
        if not args.model_ckpt or not os.path.exists(args.model_ckpt):
            raise FileNotFoundError(
                f'For --variant subgraphrag --mode infer, model checkpoint is required: {args.model_ckpt}'
            )
        model, _, device = load_model(args.model_ckpt, cpu=args.cpu)
        print(f'Loaded scorer from {args.model_ckpt}')

    out = create_dag(args, samples, embedder, model=model, device=device, verbose=args.verbose)
