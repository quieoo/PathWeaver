...
def collect_answer_node_hard_negatives(
    sample: Dict[str, Any],
    kvedges: Dict[int, KVEdge],
    ...
    answer_edge_ids: Set[int] = set()
    answer_nodes: Set[int] = set()

    for eid, e in kvedges.items():
        if value_matches_answer(e.value, answer):
            answer_edge_ids.add(eid)
            answer_nodes.add(e.dst)

    hard_neg_ids: Set[int] = set()
    for n in answer_nodes:
        for oeid in out_adj.get(n, []):
            if oeid not in answer_edge_ids:
                hard_neg_ids.add(oeid)
...
# ------------------------------------------------
        ...
        def _neg_rank_score(eid: int, feat_dict: Dict[int, Dict[str, Any]]) -> float:
            s = feat_dict[eid]['scalar']
            return float(s[3] + 0.5 * s[4] + 0.3 * s[1])  # 保持和原来一致

        # NEW: collect structural hard negatives = outgoing edges from answer nodes
        hard_neg_ids = collect_answer_node_hard_negatives(sample, kvedges, out_adj)
        # split negatives into hard negatives vs other negatives
        hard_neg_ids = [eid for eid in hard_neg_ids if eid in neg_ids]
        hard_neg_set = set(hard_neg_ids)
        other_neg_ids = [eid for eid in neg_ids if eid not in hard_neg_set]

        # rank both groups by the same semantic relevance score
        hard_neg_ids.sort(key=lambda eid: _neg_rank_score(eid, feat_dict), reverse=True)
        other_neg_ids.sort(key=lambda eid: _neg_rank_score(eid, feat_dict), reverse=True)

        max_negs = max(len(pos_ids) * args.neg_pos_ratio, args.min_negatives_per_sample)

        # NEW: only keep a capped number of hard negatives
        hard_cap = min(
            len(hard_neg_ids),
            max(args.hard_neg_min, int(len(pos_ids) * args.hard_neg_ratio))
        )

        kept_hard_neg_ids = hard_neg_ids[:hard_cap]
        remain = max_negs - len(kept_hard_neg_ids)
        kept_neg_ids = kept_hard_neg_ids + other_neg_ids[:max(0, remain)]
        ...
        for eid in kept_neg_ids:
            ...
            ys.append(0)
        # if hard_neg_ids:
        #     print(f"[hard-neg] pos={len(pos_ids)} hard_neg={len(hard_neg_ids)} kept_neg={len(kept_neg_ids)}")
# ------------------------------------------------
...
def terminal_like_postprocess(
    ...
    """
    Answer-free post-process, more aggressive and threshold-light.

    Idea:
      For an internal node n, compare:
        - best evidence for arriving at n  (best incoming edge)
        - best evidence for continuing from n (best outgoing edge)

      If continuing is not clearly better than stopping at n,
      remove outgoing edges from n.

    This version avoids strict absolute thresholds and uses mostly
    relative comparisons, so it is much more likely to trigger.
    """
    if not kept_edges:
        return kept_nodes, kept_edges
    ...
    # Small margins only; much looser than previous version
    CONTINUE_BONUS = 0.03   # best_out must exceed best_in by this much to justify continuation
    QREL_MARGIN = 0.02      # outgoing edge must also be question-relevance competitive
    MAX_ITERS = 3           # avoid over-pruning cascades
    ...
    for _ in range(MAX_ITERS):
        out_eids_by_src: Dict[int, List[int]] = defaultdict(list)
        in_eids_by_dst: Dict[int, List[int]] = defaultdict(list)
        for eid in kept_edges:
            e = kvedges[eid]
        ...
        for n in list(kept_nodes):
            in_eids = in_eids_by_dst.get(n, [])
            out_eids = out_eids_by_src.get(n, [])

            # only internal nodes
            if not in_eids or not out_eids:
                continue

            # do not aggressively stop topic anchors
            if n in topic_set:
                continue

            best_in_eid = max(in_eids, key=lambda eid: kvedges[eid].score)
            best_out_eid = max(out_eids, key=lambda eid: kvedges[eid].score)

            best_in_score = float(kvedges[best_in_eid].score)
            best_out_score = float(kvedges[best_out_eid].score)

            in_scalar = feat_dict[best_in_eid]['scalar']
            out_scalar = feat_dict[best_out_eid]['scalar']

            # existing features:
            # [1] q-dst sim
            # [4] q-value sim
            # [6] q-dst - q-src
            in_qrel = max(float(in_scalar[1]), float(in_scalar[4]))
            out_qrel = max(float(out_scalar[1]), float(out_scalar[4]))
            in_gain = float(in_scalar[6])
            out_gain = float(out_scalar[6])

            # continuation is justified only if the best outgoing edge is
            # clearly better than arriving here, both in score and question relevance
            continue_ok = (
                (best_out_score > best_in_score + CONTINUE_BONUS) and
                (out_qrel >= in_qrel - QREL_MARGIN) and
                (out_gain >= in_gain - QREL_MARGIN)
            )

            if continue_ok:
                # keep only strong competitive continuations; trim the rest
                for oeid in out_eids:
                    if oeid == best_out_eid:
                        continue
                    oe = kvedges[oeid]
                    o_scalar = feat_dict[oeid]['scalar']
                    o_score = float(oe.score)
                    o_qrel = max(float(o_scalar[1]), float(o_scalar[4]))
                    o_gain = float(o_scalar[6])

                    keep_this = (
                        o_score >= best_out_score - 0.02 and
                        o_qrel >= out_qrel - 0.02 and
                        o_gain >= out_gain - 0.03
                    )
                    if not keep_this:
                        candidate_remove.add(oeid)
            else:
                # no strong evidence to continue: make this node a sink
                for oeid in out_eids:
                    candidate_remove.add(oeid)

        if not candidate_remove:
            break

        kept_edges = {eid for eid in kept_edges if eid not in candidate_remove}
        kept_nodes, kept_edges = prune_to_reachable(topic_nodes, kept_nodes, kept_edges, kvedges)
    ...
    print(f"[terminal_like_postprocess] internal={num_internal} stop={num_stop} continue={num_continue} remove={len(candidate_remove)}")
...
def create_dag_with_model(args, samples: List[Dict[str, Any]], embedder: SentenceTransformer, model: MLPScorer, ckpt: Dict[str, Any], device: torch.device, verbose: bool = False) -> List[Dict[str, Any]]:
    ...
    for sample in tqdm(samples, desc='Create DAG (trainable SubgraphRAG-style)'):
        ...
        # NEW: push answer nodes to sinks
        if args.answer_terminalization:
            # NEW: answer-free terminalization post-process
            kept_nodes, kept_edges = terminal_like_postprocess(
                topic_nodes=topic_nodes,
...
def main():
    ...
    ap.add_argument('--hard_neg_ratio', type=float, default=0.5,
                    help='max hard negatives kept per sample = max(1, int(len(pos_ids) * hard_neg_ratio))')
    ap.add_argument('--hard_neg_min', type=int, default=1,
                    help='minimum hard negatives kept per sample if available')
