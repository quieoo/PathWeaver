...
def create_dag_with_model(
    ...
    for sample in tqdm(samples, desc='Create DAG (two-stage node-ending aware)'):
        ...
        if is_joint:
            # 联合模型推理
            edge_scores = score_edges_with_model_joint(
                edge_model, device, feat_dict, args.infer_batch_size, task='edge'
            )
            node_end_scores = score_nodes_with_model_joint(
                node_model, device, node_feat_dict, args.infer_batch_size, task='node'
            )
        else:
            # 独立模型推理
            edge_scores = score_edges_with_model(
                edge_model, device, feat_dict, args.infer_batch_size
            )
            node_end_scores = score_nodes_with_model(
                node_model, device, node_feat_dict, args.infer_batch_size
            )
