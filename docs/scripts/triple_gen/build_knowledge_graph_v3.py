...
# ============================================================
# Answer-Aware Prompt (when --answer-aware is enabled)
# ============================================================
PROMPT_PREFIX_ANSWER_AWARE_V2 = ("""
You are a knowledge graph extraction system optimized for MAXIMUM RECALL with ANSWER AWARENESS.
Your goal is to extract as many valid entities and explicit relations as possible,
while ENSURING that the answer and all supporting evidence are properly captured.

============================================================
ANSWER INFORMATION (CRITICAL)
============================================================
The correct answer for this context is: {answer}
You MUST ensure: All supporting facts that lead to the answer are captured as triples
...
def build_ctx_prompt(title: str, sentences: List[str], answer: str = None, answer_aware: bool = False) -> str:
    ctx = [title, sentences]
    ctx_json = json.dumps(ctx, ensure_ascii=False, separators=(",", ":"))
    
    if answer_aware and answer:
        # 使用 answer-aware prompt，注入答案信息
        prompt_template = PROMPT_PREFIX_ANSWER_AWARE_V2.format(answer=answer)
        return prompt_template + ctx_json
    else:
        # 使用原始 prompt
        return PROMPT_PREFIX_BIDIRECTIONAL_V2 + ctx_json
...
def flatten_samples(samples: List[Dict[str, Any]], supporting_only: bool = False) -> Tuple[List[Dict[str, Any]], Dict[int, int]]:
    ...
    for sid, s in enumerate(samples):
        ...
        answer = s.get("answer") or s.get("answer_text") or ""
        ctxs = s.get("context", [])
        kept = []
...
def flatten_samples_musique(samples: List[Dict[str, Any]], supporting_only: bool = False) -> Tuple[List[Dict[str, Any]], Dict[int, int]]:
    ...
    for sid, s in enumerate(samples):
        ...
        if s.get("answerable"):
            ...
            answer = s.get("answer") or s.get("answer_text") or ""
            if not answer:
                print(f"[WARN] Sample {sid} exists answerable but no answer.")
...
if __name__ == "__main__":
    ...
    ap.add_argument("--answer-aware", action="store_true", 
                help="Enable answer-aware extraction to ensure answer and evidence are captured")
