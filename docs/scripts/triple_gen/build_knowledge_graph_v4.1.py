...
The FIRST priority is to generate semantically complete, natural, bidirectional key_string values.
Do NOT make key_string overly short or brittle just to avoid overlap with the answer phrase.

The key_string should read like a strong natural-language retrieval key:
- semantically specific
- directionally correct
- clearly anchored
- faithful to the fact

The value_string is still only the target phrase:
- Forward KV: value_string must be exactly description
- Reverse KV: value_string must be exactly name

If a natural key_string would otherwise become too weak or unnatural, prefer the more semantically complete wording.
Minor overlap with the target anchor is acceptable in the first draft.
The pipeline can clean obvious leakage later.

Preferred forward examples:
- key="Green-breasted pitta is found in", value="DRC"
- key="the birthplace of Albert Einstein is", value="Ulm"
- key="Shirley Temple served as", value="Chief of Protocol"

Preferred reverse examples:
- key="the species found in DRC is", value="Green-breasted pitta"
- key="the person born in Ulm is", value="Albert Einstein"
- key="Chief of Protocol was held by", value="Shirley Temple"

Still avoid obviously bad keys:
- dangling templates with missing semantics
- empty or generic prefixes
- keys that change the fact meaning
