...
def eval_generate():
    ...
    kb_key_emb = torch.tensor(kb_retriever.key_embds[all_indices], dtype=torch.float32, device="cpu")
    total_kb_size = kb_key_emb.size(0)
    print(f"Knowledge base size: {total_kb_size}")
    dim = kb_key_emb.size(1)
    ...
    # all_indices_random = np.arange(len(kb_retriever_random.dataset))
    all_indices_random = np.arange(len(kb_retriever_random.dataset))
    random_key_emb = torch.tensor(kb_retriever_random.key_embds[all_indices_random], dtype=torch.float32, device="cpu")
    total_random_size = random_key_emb.size(0)
    print(f"Random knowledge base size: {total_random_size}")
    random_dim = random_key_emb.size(1)
    print(f"Random embedding dimension: {random_dim}")

    # ---------- 打印范数对比 ----------
    print("kb_embedding norm mean:", kb_key_emb.norm(dim=-1).mean().item())
    print("random_embedding norm mean:", random_key_emb.norm(dim=-1).mean().item())

    # ---------- 对随机 embedding 归一化 ----------
    # 对齐随机 embedding
    # target_norm = kb_key_emb.norm(dim=-1).mean()
    # random_key_emb = random_key_emb / (random_key_emb.norm(dim=-1, keepdim=True) + 1e-9) * target_norm
    # print("After normalization, random_embedding norm mean:", random_key_emb.norm(dim=-1).mean().item())

    # ---------- 合并两个知识库 ----------
    assert dim == random_dim, "Embedding dimension mismatch between KB and random KB."
    kb_key_emb = torch.cat([random_key_emb, kb_key_emb], dim=0)
    total_kb_size += total_random_size
    all_indices = np.arange(total_kb_size)

    # ---------- 将 CUDA tensor 转换为 CPU numpy 数组 ----------
    kb_key_emb = kb_key_emb.numpy()

    # ---------- 构建 HNSW 索引 ----------
    # 创建索引对象
    p = hnswlib.Index(space='cosine', dim=dim)  # 'l2' 表示欧氏距离，也可以是 'cosine'
    # 初始化索引
    # 参数说明：
    # - max_elements: 最大可容纳向量数
    # - ef_construction: 构建时的搜索宽度（越大越准但建得越慢）
    # - M: 每个节点的连接数（控制内存与性能的平衡）
    p.init_index(max_elements=total_kb_size, ef_construction=200, M=16)
    # 可选：设置运行时参数
    p.set_num_threads(4)  # 使用多线程
    p.set_ef(200)          # 查询时的搜索范围，越大越准但查询慢

    # ---------- 添加知识库向量 ----------
    p.add_items(kb_key_emb, all_indices)
    print(f"HNSW 索引构建完成，共 {total_kb_size} 个知识向量。")

    # ---------- 保存索引 ----------
    p.save_index("./hnsw/kb_index.bin")
